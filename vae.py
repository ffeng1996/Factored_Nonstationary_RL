...
class FNVAE:
    ...
    def __init__(self, args, logger, get_iter_idx):
        ...
        self.encoder_s = self.initialise_encoder()
        self.encoder_r = self.initialise_encoder()
        ...
        if not self.args.disable_decoder:
            if self.args.decode_reward:
                decode_rew_params = list(self.reward_decoder.parameters())
                decode_rew_params.append(self.reward_decoder.graph_SR)
                decode_rew_params.append(self.reward_decoder.graph_AR)
                decode_rew_params.append(self.reward_decoder.graph_ThetarThetar)
                decoder_params.extend(decode_rew_params)
            if self.args.decode_state:
                decode_state_params = list(self.state_decoder.parameters())
                decode_state_params.append(self.state_decoder.graph_SS)
                decode_state_params.append(self.state_decoder.graph_AS)
                decode_state_params.append(self.state_decoder.graph_ThetasS)
                decode_state_params.append(self.state_decoder.graph_ThetasThetas)
                decoder_params.extend(decode_state_params)

            if self.args.decode_task:
                decoder_params.extend(self.task_decoder.parameters())
        self.optimiser_vae = torch.optim.Adam([*self.encoder_s.parameters(), *self.encoder_r.parameters(), *decoder_params], lr=self.args.lr_vae)
    ...
    def compute_state_reconstruction_loss(self, latent, prev_latent, latent_mean_prev, latent_logvar_prev, latent_mean, latent_logvar, prev_obs, next_obs, action, elbo_indices, return_predictions=False):
        ...
        if self.args.state_pred_type == 'deterministic':
            loss_state = (state_pred - next_obs).pow(2).mean(dim=-1) + (state_pred_next - next_obs).pow(2).mean(dim=-1)
        ...
        if return_predictions:
            return loss_state, state_pred #, state_pred_next
        else:
            return loss_state

    def compute_rew_reconstruction_loss(self, latent, prev_latent, latent_mean_prev, latent_logvar_prev, latent_mean, latent_logvar, prev_obs, next_obs, action, reward, elbo_indices, return_predictions=False):
        ...
        if self.args.multihead_for_reward:
            ...
            rew_pred, rew_pred_next, latent_mean_pred, latent_logvar_pred = self.reward_decoder(latent, prev_latent, latent_mean_prev, latent_logvar_prev, next_obs, prev_obs, action.float())
            if self.args.rew_pred_type == 'bernoulli':  # TODO: untested!
                ...
                loss_rew = (rew_pred - reward).pow(2).mean(dim=-1) + (rew_pred_next - reward).pow(2).mean(dim=-1)
                kl_divergences = 0.5 * (latent_logvar_pred - latent_logvar - 1
                                        + (latent_logvar.exp() + (latent_mean - latent_mean_pred).pow(2))
                                        / latent_logvar_pred.exp())
                kl_divergences = kl_divergences.sum(dim=-1)
    ...
    def compute_loss(self, latent_mean_s, latent_logvar_s, latent_mean_s_prev, latent_logvar_s_prev,
                     latent_mean_r, latent_logvar_r, latent_mean_r_prev, latent_logvar_r_prev,
        ...
        latent_mean_s_prev = latent_mean_s_prev[:max_traj_len + 1]
        latent_logvar_s_prev = latent_logvar_s_prev[:max_traj_len + 1]
        latent_mean_r_prev = latent_mean_r_prev[:max_traj_len + 1]
        latent_logvar_r_prev = latent_logvar_r_prev[:max_traj_len + 1]
        ...
        if not self.args.disable_stochasticity_in_latent:
            ...
            latent_samples_s_prev = self.encoder_s._sample_gaussian(latent_mean_s_prev, latent_logvar_s_prev)
            latent_samples_r_prev = self.encoder_r._sample_gaussian(latent_mean_r_prev, latent_logvar_r_prev)
        else:
            ...
            latent_samples_s_prev = torch.cat((latent_mean_s_prev, latent_logvar_s_prev), dim=-1)
            latent_samples_r_prev = torch.cat((latent_mean_r_prev, latent_logvar_r_prev), dim=-1)

        num_elbos = latent_samples_s.shape[0]
        num_decodes = vae_prev_obs.shape[0]
        batchsize = latent_samples_s.shape[1]  # number of trajectories
        ...
        if self.args.vae_subsample_elbos is not None:
            ...
            latent_samples_s = latent_samples_s[elbo_indices, task_indices, :].reshape((self.args.vae_subsample_elbos, batchsize, -1))
            latent_samples_r = latent_samples_r[elbo_indices, task_indices, :].reshape(
                (self.args.vae_subsample_elbos, batchsize, -1))
            latent_samples_s_prev = latent_samples_s_prev[elbo_indices, task_indices, :].reshape(
                (self.args.vae_subsample_elbos, batchsize, -1))
            latent_samples_r_prev = latent_samples_r_prev[elbo_indices, task_indices, :].reshape(
                (self.args.vae_subsample_elbos, batchsize, -1))
            num_elbos = latent_samples_s.shape[0]
        else:
            elbo_indices = None
        ...
        dec_embedding_s_prev = latent_samples_s_prev.unsqueeze(0).expand((num_decodes, *latent_samples_s_prev.shape)).transpose(1, 0)
        dec_embedding_r_prev = latent_samples_r_prev.unsqueeze(0).expand((num_decodes, *latent_samples_r_prev.shape)).transpose(1, 0)
        ...
        if self.args.decode_reward:
            ...
            rew_reconstruction_loss = self.compute_rew_reconstruction_loss(dec_embedding_r, dec_embedding_r_prev,
                                                                           latent_mean_r_prev, latent_logvar_r_prev,
                                                                           latent_mean_r, latent_logvar_r,
                                                                           dec_prev_obs, dec_next_obs,
                                                                           dec_actions, dec_rewards, elbo_indices)
        ...
        if self.args.decode_state:
            state_reconstruction_loss = self.compute_state_reconstruction_loss(dec_embedding_s, dec_embedding_s_prev,
                                                                               latent_mean_s_prev, latent_logvar_s_prev,
                                                                               latent_mean_s, latent_logvar_s,
                                                                               dec_prev_obs,
                                                                               dec_next_obs, dec_actions, elbo_indices)
            # avg/sum across individual ELBO terms
            if self.args.vae_avg_elbo_terms:
        ...
        if not self.args.disable_kl_term:
            ...
            kl_loss = self.compute_kl_loss(latent_mean_s, latent_logvar_s, elbo_indices) + self.compute_kl_loss(latent_mean_r, latent_logvar_r, elbo_indices)
            # avg/sum the elbos
            if self.args.vae_avg_elbo_terms:
        ...
        sparsity_loss = (torch.norm(self.state_decoder.graph_SS, p=1) + torch.norm(self.state_decoder.graph_AS, p=1) +
                         torch.norm(self.state_decoder.graph_ThetasS, p=1) +
                         torch.norm(self.state_decoder.graph_ThetasThetas, p=1) +
                         torch.norm(self.reward_decoder.graph_SR, p=1) + torch.norm(self.reward_decoder.graph_AR, p=1) +
                         torch.norm(self.reward_decoder.graph_ThetarThetar, p=1))
        sparsity_loss += (torch.norm(latent_samples_s - latent_samples_s_prev, p=1) + torch.norm(latent_samples_r - latent_samples_r_prev, p=1))
        return rew_reconstruction_loss, state_reconstruction_loss, task_reconstruction_loss, kl_loss, sparsity_loss

    def compute_loss_split_batches_by_elbo(self, latent_mean_s, latent_logvar_s, latent_mean_r, latent_logvar_r, vae_prev_obs, vae_next_obs, vae_actions,
                                           vae_rewards, vae_tasks, trajectory_lens):
        ...
        n_elbos = latent_mean_s.shape[0]  # includes the prior
        ...
        for idx_elbo in range(n_elbos):
            ...
            if not self.args.decode_only_past:
                ...
                dec_embedding_r = curr_samples_s.unsqueeze(0).expand((n_horizon, *curr_samples_r.shape))
            ...
            if self.args.decode_reward:
                ...
                rrc = self.compute_rew_reconstruction_loss(dec_embedding_r, dec_prev_obs, dec_next_obs, dec_actions,
                                                           dec_rewards)
            ...
            if self.args.decode_state:
                src = self.compute_state_reconstruction_loss(dec_embedding_s, dec_prev_obs, dec_next_obs, dec_actions)
                # sum up the reconstruction terms; average over tasks
        ...
        if not self.args.disable_kl_term:
            # compute the KL term for each ELBO term of the current trajectory
            kl_loss = self.compute_kl_loss(latent_mean_s, latent_logvar_s, None) + self.compute_kl_loss(latent_mean_r, latent_logvar_r, None)
    ...
    def compute_vae_loss(self, update=False, pretrain_index=None, freeze=False):
        ...
        #print(latent_mean_s.shape, latent_logvar_s.shape) #[401, 10, 5],
        latent_zeros = torch.zeros_like(latent_mean_s[0, :, :]).to(latent_mean_s.device)
        latent_zeros = latent_zeros.unsqueeze(0)
        latent_mean_s_prev = torch.cat((latent_zeros, latent_mean_s[:-1, :, :]), dim=0)
        latent_logvar_s_prev = torch.cat((latent_zeros, latent_logvar_s[:-1, :, :]), dim=0)
        ...
        latent_mean_r_prev = torch.cat((latent_zeros, latent_mean_r[:-1, :, :]), dim=0)
        latent_logvar_r_prev = torch.cat((latent_zeros, latent_logvar_r[:-1, :, :]), dim=0)

        if self.args.split_batches_by_elbo:
            losses = self.compute_loss_split_batches_by_elbo(latent_mean_s, latent_logvar_s,latent_mean_r, latent_logvar_r, vae_prev_obs, vae_next_obs,
                                                             vae_actions, vae_rewards, vae_tasks,
                                                             trajectory_lens)
        else:
            losses = self.compute_loss(latent_mean_s, latent_logvar_s, latent_mean_s_prev, latent_logvar_s_prev, latent_mean_r, latent_logvar_r, latent_mean_r_prev, latent_logvar_r_prev, vae_prev_obs, vae_next_obs, vae_actions,
                                       vae_rewards, vae_tasks, trajectory_lens)
        rew_reconstruction_loss, state_reconstruction_loss, task_reconstruction_loss, kl_loss, sparsity_loss = losses
