...
class HalfCheetahEnv(HalfCheetahEnv_):
    ...
    def visualise_behaviour(env,
        ...
        if encoder_s is not None:
            episode_latentS_samples = [[] for _ in range(num_episodes)]
            episode_latentS_means = [[] for _ in range(num_episodes)]
            episode_latentS_logvars = [[] for _ in range(num_episodes)]
        else:
            curr_latentS_sample = curr_latentS_mean = curr_latentS_logvar = None
            episode_latentS_samples = episode_latentS_means = episode_latentS_logvars = None
        ...
        for episode_idx in range(num_episodes):
            ...
            if encoder_s is not None:
                if episode_idx == 0:
                    curr_latentS_sample, curr_latentS_mean, curr_latentS_logvar, hidden_state = encoder_s.prior(1)
                    curr_latentS_sample = curr_latentS_sample[0].to(device)
                    curr_latentS_mean = curr_latentS_mean[0].to(device)
                    curr_latentS_logvar = curr_latentS_logvar[0].to(device)
                episode_latentS_samples[episode_idx].append(curr_latentS_sample[0].clone())
                episode_latentS_means[episode_idx].append(curr_latentS_mean[0].clone())
                episode_latentS_logvars[episode_idx].append(curr_latentS_logvar[0].clone())
