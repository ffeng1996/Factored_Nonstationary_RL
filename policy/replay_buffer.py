...
class ReplayBuffer:
    ...
    def __init__(self, state_space, theta_space, action_space, max_size=100000):
        self.current_states = np.empty((0, state_space), dtype=np.float64)
        self.current_theta = np.empty((0, theta_space), dtype=np.float64)
    ...
    def store(self, current_state, current_theta, action, reward, next_state, end):
        self.current_states = np.append(self.current_states[-self.max_size:],
                                        np.array(current_state, ndmin=2), axis=0)
        self.current_theta = np.append(self.current_theta[-self.max_size:],
                                        np.array(current_theta, ndmin=2), axis=0)
    ...
    def fetch_sample(self, num_samples):
        ...
        current_theta_ = self.current_states[idx]
        actions_ = self.actions[idx]
        rewards_ = self.rewards[idx]
        next_states_ = self.next_states[idx]
        ends_ = self.ends[idx]

        return current_states_, current_theta_, actions_, rewards_, next_states_, ends_
