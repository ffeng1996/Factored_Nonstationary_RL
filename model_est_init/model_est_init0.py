...
class missVAE(object):
    ...
    def build_model(self):        
        ...
        with tf.variable_scope('SSL', reuse=tf.AUTO_REUSE):
            self.SSL_A = tf.get_variable('A', [self.hps.z_s_size], tf.float32,
                                            initializer=tf.contrib.layers.xavier_initializer())
            self.SSL_B = tf.get_variable('B', [self.hps.z_r_size], tf.float32,
                                            initializer=tf.contrib.layers.xavier_initializer())
            self.SSL_C = tf.get_variable('C', [self.hps.action_size], tf.float32,
                                            initializer=tf.contrib.layers.xavier_initializer())
            self.SSL_D1 = tf.get_variable('D1', [self.hps.z_s_size, self.hps.z_s_size], tf.float32,
                                            initializer=tf.contrib.layers.xavier_initializer())
            self.SSL_D2 = tf.get_variable('D2', [self.hps.z_r_size, self.hps.z_r_size], tf.float32,
                                            initializer=tf.contrib.layers.xavier_initializer())
            self.SSL_E = tf.get_variable('E', [self.hps.action_size, self.hps.o_size], tf.float32,
                                            initializer=tf.contrib.layers.xavier_initializer())
            self.SSL_F = tf.get_variable('F', [self.hps.o_size, self.hps.o_size], tf.float32,
                                            initializer=tf.contrib.layers.xavier_initializer())
            self.SSL_G = tf.get_variable('G', [self.hps.o_size], tf.float32,
                                            initializer=tf.contrib.layers.xavier_initializer())
        ...
        with tf.variable_scope('Encoder', reuse=tf.AUTO_REUSE):
            # obs_x
            hx = tf.layers.dense(obs_x, units=256, activation=tf.nn.relu)
            hx = tf.layers.dense(hx, units=256, activation=tf.nn.relu)

            # obs_a
            ha = tf.layers.dense(obs_a_prev, units=256, activation=tf.nn.relu, name="enc_action_fc1")
            ha = tf.layers.dense(ha, units=256, activation=tf.nn.relu, name="enc_action_fc2")

            # obs_r
            hr = tf.layers.dense(obs_r_prev, units=256, activation=tf.nn.relu, name="enc_reward_fc1")
            hr = tf.layers.dense(hr, units=256, activation=tf.nn.relu, name="enc_reward_fc2")
        ...
        with tf.variable_scope('ObsDecoder', reuse=tf.AUTO_REUSE):
            h = tf.layers.dense(tf.concat([ssl_zo, ssl_ao], 1), 512, kernel_constraint=self.deconv_weightnorm, name="dec_fc1")
            h = tf.layers.dense(h, 512, kernel_constraint=self.deconv_weightnorm, name="dec_fc2")
            self.y_o = tf.layers.dense(h, self.hps.o_size, kernel_constraint=self.deconv_weightnorm, name="dec_fc3")
        ...
        with tf.variable_scope('NextObsDecoder', reuse=tf.AUTO_REUSE):
            nh_s = tf.layers.dense(tf.concat([obs_a, obs_x], 1), 512, kernel_constraint=self.deconv_weightnorm, name="dec_fc1")
            nh_s = tf.layers.dense(nh_s, 512, kernel_constraint=self.deconv_weightnorm, name="dec_fc2")
            nh_s = tf.layers.dense(nh_s, self.hps.o_size, kernel_constraint=self.deconv_weightnorm, name="dec_fc3")
        ...
        with tf.variable_scope('RewardDecoder', reuse=tf.AUTO_REUSE):
            lin_h = tf.layers.dense(ssl_zar, 512, activation=tf.nn.relu,
                                        kernel_constraint=self.deconv_weightnorm, name="dec_fc1")
            lin_h = tf.layers.dense(lin_h, 512, activation=tf.nn.relu,
        ...
        with tf.variable_scope('NextRewardDecoder', reuse=tf.AUTO_REUSE):
            lin_h_next = tf.layers.dense(ssl_rf, 512, activation=tf.nn.relu,
                                        kernel_constraint=self.deconv_weightnorm, name="dec_fc1")
            lin_h_next = tf.layers.dense(lin_h_next, 512, activation=tf.nn.relu,
                                    kernel_constraint=self.deconv_weightnorm, name="dec_fc2")
        ...
        if self.hps.is_training == 1:
            ...
            self.r_reward_next_loss = tf.reduce_mean(self.r_reward_next_loss)
            # KL loss
            ...
            self.vae_loss = .8 * self.r_obs_loss + .8 * self.r_next_obs_loss + .8 * self.r_reward_loss + .5 * self.kl_loss + \
                            self.vae_causal_filter_loss
            self.vae_pure_loss = .8 * self.r_obs_loss + .8 * self.r_next_obs_loss + .8 * self.r_reward_loss + .5 * self.kl_loss 
            ...
            self.total_loss = self.vae_pure_loss + self.causal_filter_loss + .02 * self.smooth_loss
            ############################################## Optimizers ##########################################            
            ...
            for grad, var in vae_gvs:
                if not (var.name.startswith('SSL/A') or var.name.startswith('SSL/B') or var.name.startswith('SSL/C') or var.name.startswith('SSL/D1') or var.name.startswith('SSL/D2') or var.name.startswith('SSL/E') or var.name.startswith('SSL/F') or var.name.startswith('SSL/G') or var.name.startswith('Dynamics')):
    ...
    def markovian_tran(self):
        ...
        with tf.variable_scope('Dynamics', reuse=tf.AUTO_REUSE):
            hd = tf.layers.dense(random_ssl_za, 512, activation=tf.nn.relu,
                                 kernel_constraint=self.deconv_weightnorm, name="fc1")
            hd = tf.layers.dense(hd, 512, activation=tf.nn.relu,
